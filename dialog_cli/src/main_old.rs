    ...
    match matches.subcommand() {
        ...
        Some(("create-group", sub_matches)) => {
            ...
            if let Some(key_package_event) = events.first() {
                ...
                for rumor in welcome_rumors {
                    ...
                    let gift_wrap_event = EventBuilder::gift_wrap(&keys, &counterparty_pk, rumor.clone(), None)
                        .await
                        .map_err(|e| DialogError::General(Box::new(e)))?;
                    ...
                    let output = client
                        ...
                        .await
                        .map_err(|e| DialogError::General(Box::new(e)))?;
                    println!("Published gift-wrapped welcome event ID: {:?}", output.id());
                    ...
                    println!("Publishing regular MLS welcome event: {}", welcome_event.id);
                    let output = client
                        .send_event(&welcome_event)
                        .await
                        .map_err(|e| DialogError::General(Box::new(e)))?;
                    println!("Published regular MLS welcome event ID: {:?}", output.id());
                }
            } else {
        ...
        Some(("list-invites", sub_matches)) => {
            ...
            let giftwrap_events = client
                .fetch_events(giftwrap_filter, std::time::Duration::from_secs(10))
                .await
                .map_err(|e| DialogError::General(Box::new(e)))?;

            println!("Found {} gift-wrapped events", giftwrap_events.len());
            ...
            let welcome_filter = Filter::new().kind(Kind::MlsWelcome);
            let welcome_events = client
                .fetch_events(welcome_filter, std::time::Duration::from_secs(10))
                .await
                .map_err(|e| DialogError::General(Box::new(e)))?;

            println!("Found {} regular MLS welcome events", welcome_events.len());

            // Process regular MLS welcome events directly
            for event in welcome_events {
                // Process the MLS welcome event directly (no gift wrap unwrapping needed)
                if let Err(e) = nostr_mls.process_message(&event) {
                    // Ignore errors - the event might not be for us or might be malformed
                    tracing::debug!("Failed to process MLS welcome from {}: {}", event.pubkey, e);
                }
            }
